...
module.exports = function(app, forumData) {
    ...
    app.get('/viewposts',function(req,res){
        ...
        let sqlquery = `SELECT   u.username,u.user_id,post_id, p.post_date, t.topic_title, p.post_title, p.post_content
                        FROM     posts p
                        JOIN     topics t
    ...
    app.get('/search-result', function (req, res) {
        ...
        let sqlquery = `SELECT   post_id, p.post_date, t.topic_title, p.post_title, p.post_content,u.username
                        FROM     posts p
                        JOIN     topics t
                        ON       t.topic_id=p.topic_id JOIN users u ON u.user_id=p.user_id
...
console.log(result)
            res.render('viewposts',{posts:result,forumName:forumData.forumName})
        });      
    });

app.get('/user/detail/:id',function(req,res){
    var user_id=req.params.id;
  sqlquery = `Select u.username,u.firstname , u.country, p.post_title,p.post_date ,t.topic_title from users u RIGHT OUTER JOIN posts p ON u.user_id = p.user_id RIGHT OUTER JOIN topics t ON t.topic_id =p.topic_id where u.user_id =?;`;
                db.query(sqlquery, [user_id], (err, result) => {
                    if (err) {
                        return console.error(err.message);
                    }
                   
                     res.render('userdetails',{posts:result,forumName:forumData.forumName})
                })
            })
            app.get('/topic/detail/:id',function(req,res){
         var topic_id=req.params.id;
         sqlquery = `Select u.*, t.topic_title from users u, topics t where t.topic_id=1 and u.user_id in (SELECT p.user_id from posts p where p.topic_id=?)`;
                db.query(sqlquery, [topic_id], (err, result) => {
                    if (err) {
                        return console.error(err.message);
                    }
                   
                     res.render('topicdetails',{users:result,forumName:forumData.forumName})
                })
            })
           
          app.get('/postsfromuser/:id',function(req,res){
              var user_id=req.params.id
         let sqlquery = `SELECT   u.username,post_id, p.post_date, t.topic_title, p.post_title, p.post_content
                        FROM     posts p
                        JOIN     topics t
                        ON       t.topic_id=p.topic_id
                        JOIN users u ON u.user_id= p.user_id
